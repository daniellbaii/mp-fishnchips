import PageLayout from '@/components/layout/PageLayout';

export default function MenuPage() {
  return (
    <PageLayout>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="text-center">
          <h1 className="text-4xl font-bold text-gray-800 mb-4">Menu & Online Ordering</h1>
          <p className="text-xl text-gray-500 mb-8">Fresh seafood, expertly prepared, delivered fast</p>
          <div className="bg-white p-8 rounded-lg shadow-card">
            <p className="text-gray-500">Our full menu and online ordering system is coming soon!</p>
            <p className="text-gray-500 mt-4">Call us at (08) 9123 4567 to place your order.</p>
          </div>
        </div>
      </div>
    </PageLayout>
  );
}